...
public:
  ...
  TriangulationGraphicsItem(T* t_, bool is_constrained);
...
template <typename T>
TriangulationGraphicsItem<T>::TriangulationGraphicsItem(T * t_, bool is_constrained)
...
template <typename T>
void TriangulationGraphicsItem<T>::operator()(typename T::Face_handle fh)
{
  if(visible_edges) {
    for (int i=0; i<3; i++) {
...
template <typename T>
void TriangulationGraphicsItem<T>::drawAll(QPainter *painter)
{
...
template <typename T>
void TriangulationGraphicsItem<T>::paintVertices(QPainter *painter)
{
  if (visibleVertices()) {
...
template <typename T>
void TriangulationGraphicsItem<T>::paintOneVertex(const typename T::Point& point)
{
  Converter<Geom_traits> convert;
...
template <typename T>
void TriangulationGraphicsItem<T>::paintVertex(typename T::Vertex_handle vh)
{
  Converter<Geom_traits> convert;
...
template <typename T>
void TriangulationGraphicsItem<T>::paint(QPainter *painter,
...
template <typename T>
void TriangulationGraphicsItem<T>::updateBoundingBox()
{
...
template <typename T>
void TriangulationGraphicsItem<T>::modelChanged()
{
  if (t->number_of_vertices() == 0) {
    this->hide();
