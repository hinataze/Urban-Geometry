...
static std::string s_locate = R"(<br>   Face_handle locate (const Point& p, Locate_type& lt, int& li, Face_handle start = Face_handle()) const
 ...
 <br>                 return generic_locate(p, lt, li, start, Should_filter_tag());
 <br>           }

 <br>           Please check the complete code in file "Triangulation_2.h".Copy and paste the portion of the code
 <br>           You can get it from CGAL on github at <a href="https://github.com/CGAL/cgal/blob/master/Triangulation_2/include/CGAL/Triangulation_2.h"> https://github.com/CGAL/cgal/blob/master/Triangulation_2/include/CGAL/Triangulation_2.h </a>
...
public:
  ...
  locate(const Point & p,
         ...
         Face_handle start = Face_handle()) const
  {
      hs_update_log("locate");
      hs_vps.push_back(std::pair <std::string, std::string>("", s_locate));
